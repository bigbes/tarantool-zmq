...
function _M.zmq_msg_data(msg, offset)
  local ptr = libzmq3.zmq_msg_data(msg)
  if offset == nil or offset == 0 then
    return ptr
  end
  return ffi.cast(pvoid_t, ffi.cast(pchar_t, ptr) + offset)
end

function _M.zmq_msg_data_str(msg, offset, limit)
  offset = offset or 0
  local ptr = ffi.cast(pchar_t, libzmq3.zmq_msg_data(msg))
  local ptr_size = libzmq3.zmq_msg_size(msg)
  if (limit and limit + offset > ptr_size) or (not limit) then
      limit = ptr_size - offset
  end
  return ffi.string(ptr + offset, limit)
end
...
do
  local ERRORS_MNEMO = {}
  for errname, errnumber in pairs(getmetatable(errno).__index) do
      ERRORS_MNEMO[errnumber] = errname
  end
